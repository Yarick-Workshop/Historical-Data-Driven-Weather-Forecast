...
internal sealed class NeuralNetworkForecastProcessor : IDisposable
{
    private const int BaseFeatureCount = 6;

    private readonly ForecastOptions _options;
    private readonly NeuralNetworkType _networkType;
    private readonly Device _device;
    private IRegressionModel? _model;
    private TrainingStats? _lastTrainingStats;

    public NeuralNetworkForecastProcessor(ForecastOptions options, NeuralNetworkType networkType)
    {
        _options = options;
        ...
        var networkTypeName = GetNetworkTypeName();
        Log.Information("  [{NetworkType}] Configured to use {ThreadCount} CPU thread(s) for training (out of {AvailableCores} available).", networkTypeName, numThreads, availableCores);
    }
    ...
    public ForecastResult Process(IReadOnlyList<WeatherObservation> rawObservations)
    {
        ...
        {
            return CreateEmptyResult(ordered, trainingRows.Count, validationRows.Count, $"insufficient total observations for {networkTypeName}.");
        }

        if (trainingRows.Count <= _options.WindowSize)
        {
            return CreateEmptyResult(ordered, trainingRows.Count, validationRows.Count, $"insufficient training rows for {networkTypeName} (need more history than the window size).");
        }
        ...
        Log.Debug("  [{NetworkType}] Built {SequenceCount} training sequence(s) in {Elapsed}.",
            networkTypeName,
        ...
        Log.Information("  [{NetworkType}] Training completed in {Elapsed}.", networkTypeName, FormatDuration(trainingStopwatch.Elapsed));
        ...
        Log.Information("  [{NetworkType}] Evaluation (validation + forecasting) completed in {Elapsed}.",
            networkTypeName,
            FormatDuration(evaluationStopwatch.Elapsed));

        totalStopwatch.Stop();
        Log.Information("  [{NetworkType}] Dataset processing time: {Elapsed}.", networkTypeName, FormatDuration(totalStopwatch.Elapsed));
    ...
    public NeuralNetworkType NetworkType => _networkType;

    private string GetNetworkTypeName()
    {
        return _networkType switch
        {
            NeuralNetworkType.LSTM => "LSTM",
            NeuralNetworkType.GRU => "GRU",
            _ => _networkType.ToString().ToUpperInvariant()
        };
    }
    ...
    {
        _model?.Dispose();
        _model = _networkType switch
        {
            NeuralNetworkType.LSTM => new LstmRegressionModel(featureCount, _options.HiddenSize, 1),
            NeuralNetworkType.GRU => new GruRegressionModel(featureCount, _options.HiddenSize, 1),
            _ => throw new NotSupportedException($"Network type {_networkType} is not supported.")
        };
    }
	...
	private List<EpochStat> TrainModel(List<SequenceSample> trainingSequences, NormalizationStatistics stats)
    {
        ...
        var sequenceLength = trainingSequences[0]?.Sequence.Length ?? 0;
        if (sequenceLength == 0 || trainingSequences.Count == 0)
        {
            throw new InvalidOperationException("Training sequences cannot be empty.");
        }
        var networkTypeName = GetNetworkTypeName();
        ...
        Log.Information("  [{NetworkType}] Training {SequenceCount} sequence(s) with {FeatureCount} feature(s) each, batch size {BatchSize}, epochs {Epochs}.",
            networkTypeName, trainingSequences.Count, featureCount, _options.BatchSize, _options.TrainingEpochs);
        Log.Information("  [{NetworkType}] Total batches per epoch: {BatchesPerEpoch}, Total training steps: {TotalSteps}",
            networkTypeName, totalBatchesPerEpoch, totalSteps);
        ...
        for (var epoch = 1; epoch <= _options.TrainingEpochs; epoch++)
        {
            ...
            for (var batchStart = 0; batchStart < indices.Length; batchStart += _options.BatchSize)
            {
                ...
                if (stepCounter % 100 == 0)
                {
                    ...
                    Log.Information("  [{NetworkType}] ========== Training Progress Report (Step {Step}/{TotalSteps}) ==========", networkTypeName, stepCounter, totalSteps);
                    Log.Information("  [{NetworkType}] Progress: {Progress:F2}% | Epoch {Epoch}/{TotalEpochs} | Batch {Batch}/{TotalBatches} | Sequence Length: {SeqLength} | Features: {Features}",
                        networkTypeName, progressPercent, epoch, _options.TrainingEpochs, batchCount, totalBatchesPerEpoch, sequenceLength, featureCount);
                    Log.Information("  [{NetworkType}] Loss Metrics:", networkTypeName);
                    Log.Information("  [{NetworkType}]   Current Batch Loss:     {CurrentLoss:F8}", networkTypeName, currentLoss);
                    Log.Information("  [{NetworkType}]   Epoch Average Loss:     {EpochAvgLoss:F8} (over {BatchCount} batches)", networkTypeName, epochAvgLoss, batchCount);
                    Log.Information("  [{NetworkType}]   Overall Average Loss:   {AvgLoss:F8} (over {TotalSteps} steps)", networkTypeName, avgLossSoFar, cumulativeStepCount);
                    if (!double.IsNaN(epochLossChange))
                    {
                        Log.Information("  [{NetworkType}]   Epoch Loss Change:      {Change:F2}% vs previous epoch", networkTypeName, epochLossChange);
                    }
                    if (!double.IsNaN(epochProgressLossChange))
                    {
                        Log.Information("  [{NetworkType}]   Epoch Progress Change:  {Change:F2}% vs epoch start", networkTypeName, epochProgressLossChange);
                    }
                    if (!double.IsNaN(lossTrend))
                    {
                        Log.Information("  [{NetworkType}]   Recent Loss Trend:      {Trend:F2}% (last 25 vs first 25 of recent 50)", networkTypeName, lossTrend);
                    }
                    Log.Information("  [{NetworkType}] Training Configuration:", networkTypeName);
                    Log.Information("  [{NetworkType}]   Learning Rate:          {LearningRate:E4}", networkTypeName, _options.LearningRate);
                    Log.Information("  [{NetworkType}]   Batch Size:              {BatchSize} | Actual Batch: {ActualBatchSize}", networkTypeName, _options.BatchSize, batchIndices.Length);
                    Log.Information("  [{NetworkType}]   Hidden Size:             {HiddenSize}", networkTypeName, _options.HiddenSize);
                    if (gradientNorm.HasValue)
                    {
                        Log.Information("  [{NetworkType}]   Gradient Norm:           {GradNorm:F6}", networkTypeName, gradientNorm.Value);
                    }
                    Log.Information("  [{NetworkType}] Performance Metrics:", networkTypeName);
                    var batchesPerSecond = elapsed.TotalSeconds > 0 
                        ? cumulativeStepCount / elapsed.TotalSeconds 
                        : 0.0;
                    Log.Information("  [{NetworkType}]   Training Speed:          {StepsPerSec:F2} steps/sec ({BatchesPerSec:F2} batches/sec)",
                        networkTypeName, stepsPerSecond, batchesPerSecond);
                    Log.Information("  [{NetworkType}]   Elapsed Time:            {Elapsed}", networkTypeName, FormatDuration(elapsed));
                    Log.Information("  [{NetworkType}]   Estimated Time Remaining: {ETA}", networkTypeName, FormatDuration(estimatedTimeRemaining));
                    ...
                    var epochETA = batchCount > 0 && epochElapsed.TotalSeconds > 0
                        ...
                        : TimeSpan.Zero;
                    Log.Information("  [{NetworkType}]   Epoch Elapsed:           {EpochElapsed} | Epoch ETA: {EpochETA}",
                        networkTypeName, FormatDuration(epochElapsed), FormatDuration(epochETA));
                    Log.Information("  [{NetworkType}] =================================================================", networkTypeName);
                }
            }
            ...
            if (batchCount > 0)
            {
                ...
                Log.Debug("  [{NetworkType}] Epoch {Epoch}/{TotalEpochs} completed in {Elapsed} - Batches: {BatchCount}, AvgLoss: {AverageLoss:F6}",
                    networkTypeName, epoch, _options.TrainingEpochs, FormatDuration(epochStopwatch.Elapsed), batchCount, avgLoss);
            }
            else
            {
                epochStats.Add(new EpochStat(epoch, 0, double.NaN, FormatDuration(epochStopwatch.Elapsed)));
                Log.Warning("  [{NetworkType}] Epoch {Epoch}/{TotalEpochs}, no batches processed.", networkTypeName, epoch, _options.TrainingEpochs);
            }
        }
        
        trainingStopwatch.Stop();
        var finalAvgLoss = cumulativeLoss / cumulativeStepCount;
        Log.Information("  [{NetworkType}] Training Summary - Total Steps: {TotalSteps} | Final Average Loss: {FinalAvgLoss:F6} | Total Time: {TotalTime}",
            networkTypeName, cumulativeStepCount, finalAvgLoss, FormatDuration(trainingStopwatch.Elapsed));
        return epochStats;
    }
    ...
    private List<SequenceSample> BuildSequences(IReadOnlyList<WeatherObservation> rows, NormalizationStatistics stats)
    {
        ...
        foreach (var observation in rows)
        {
            ...
            if (lastLoggedDay != day)
            {
                ...
                {
                    Log.Debug("  [{NetworkType}] Training progress: processed {DayCount} day(s); current day {Date}",
                        networkTypeName, distinctDayCount, day);
                }
            }
        ...
        Log.Information("  [{NetworkType}] Training set covers {TotalDays} distinct day(s).", networkTypeName, distinctDayCount);
    ...
    private interface IRegressionModel : IDisposable
    {
        Tensor Forward(Tensor input);
        IEnumerable<Parameter> Parameters();
    }

    private sealed class LstmRegressionModel : IRegressionModel
    {
    ...
    private sealed class GruRegressionModel : IRegressionModel
    {
        private readonly GRU _gru;
        private readonly Linear _linear;

        public GruRegressionModel(int inputSize, int hiddenSize, int numLayers)
        {
            _gru = nn.GRU(inputSize, hiddenSize, numLayers: numLayers, batchFirst: true);
            _linear = nn.Linear(hiddenSize, 1);
        }

        public Tensor Forward(Tensor input)
        {
            var (output, _) = _gru.forward(input);
            var last = output.slice(1, output.shape[1] - 1, output.shape[1], 1);
            last = last.squeeze(1);
            return _linear.forward(last);
        }
        ...
        public IEnumerable<Parameter> Parameters()
        {
            foreach (var parameter in _gru.parameters())
            {
                yield return parameter;
            }
